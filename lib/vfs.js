...
function getFile(path,id){
	try{
		...
		var end = tryNames.length;
		
		//console.log(fileName,tryNames,dirpath)
	}catch(e){
...
function getDirectoryInfo(path){
	...
	if(!dir){
		...
		dirMap[path] = dir = {
			...
			fileMap:stat && _readFileMap(path)
		}
		_addDirWatch(path);
	}
	return dir;
		...
		if((tryName in newFileMap) && !(tryName in oldFileMap)){//new File
			...
			for(var trueName in hitMap){
				...
				for(var hitId in hitPathMap){
					if(hitPathMap[hitId] in hitMap){//TODO:....
...
function _on_file_change(dir){//file modified,delete
	...
	for(var trueName in hitMap){
		try{
			...
			console.info('file changed',absPath)
		}catch(e){
...
function _clean_cache(hitMap,dirPath,trueName){
	...
	console.info('clean cache:',trueName)
	for(var id in pathMap){
		console.info('cache id:',id)
