...
function addAction(callback,pattern){
	...
	var server = require('http').createServer(function (req, response) {
		...
		while(ai--){
			...
			if(a[0] == null && a[1](req,response)){
				...
				console.log('execute action:',req.url,a[1].name || a[0])
...
function writeContent(req,response,url,text){
	...
	response.write(text);
	var wait = serverExt.getBlockTime(url);
	if(wait >=0){
...
function writeDir(url,filepath,response){
	if(/\/$/.test(url)){
		FS.readdir(filepath, function(err, files) {  
			...
			response.write('<meta content="text/html; charset=UTF-8" http-equiv="content-type"><span>'+filepath+
				"</span>&#160;<a href='../' title='返回上一级'>[↑]</a>&#160;<a href='/--export.zip' title='导出'>[>=]</a><hr/>",'utf8');
